...
public class GhostscriptConverter : Converter
{
    ...
    async public override Task ConvertFile(FileToConvert file)
	{
		...
		string? extension = GetExtension(file.TargetPronom);
		string? sDevice = GetDevice(file.TargetPronom);

		if(extension == null || sDevice == null)
		{
			Logger.Instance.SetUpRunTimeLogMessage(file.TargetPronom + " is not supported by GhostScript. File is not converted.", true,filename: file.FilePath);
			return;
		}
		...
		try
		{
            if (extension == ".pdf")
            {
                string pdfVersion = GetPDFVersion(file.TargetPronom);
				lock (lockobject)
				{
		...
		{
			Logger.Instance.SetUpRunTimeLogMessage(file.TargetPronom + " is not supported by GhostScript. File is not converted.", true, file.FilePath);
		}
	}
    ...
    void convertToImagesWindows(FileToConvert file, string outputFileName, string sDevice, string extension)
	{
		...
		try
		{
			...
			do {
				...
				{
					converted = CheckConversionStatus(newFile.FilePath, file.TargetPronom);
        ...
        try
        {
			...
			do
			{
				...
				foreach (var filePath in newFilePaths)
				{
					...
					newFile.AddConversionTool(NameAndVersion);
					files.Add(newFile);
                    converted = converted && CheckConversionStatus(newFile.FilePath, file.TargetPronom);
                }
            } while (!converted && ++count < GlobalVariables.MAX_RETRIES);
			FileManager.Instance.AddFiles(files);
            file.Failed = !converted;
	...
	void convertToPDF(FileToConvert file, string outputFileName, string sDevice, string extension, string pdfVersion)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                if (currPronom != file.TargetPronom && 
						(PDFPronoms.Contains(file.TargetPronom) || PDFAPronoms.Contains(file.TargetPronom) ))
                {
                    ...
                    converter.convertFromPDFToPDF(file, file.TargetPronom);
                }
                converted = CheckConversionStatus(outputFilePath, file.TargetPronom);
            } while (!converted && ++count < GlobalVariables.MAX_RETRIES);
            if (!converted)
            {
                file.Failed = true;
