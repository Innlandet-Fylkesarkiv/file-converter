...
public class iText7 : Converter
{
    ...
    public override void CombineFiles(List<FileInfo> files)
    {
        ...
        {
            Logger.Instance.SetUpRunTimeLogMessage("Error combining files with iText7. Error message: " + e.Message, true, files.First().TargetPronom, Path.GetDirectoryName(files.First().FilePath) ?? files.First().FilePath);
        }
    }
    ...
    Task MergeFilesToPDF(List<FileInfo> files, string outputFileName)
     {
        try
        {
            PdfVersion? pdfVersion = PronomToPdfVersion[files.First().TargetPronom];
            if (pdfVersion == null)
            {
                pdfVersion = PdfVersion.PDF_2_0; //Default PDF version
            }
            ...
            PdfAConformanceLevel? conformanceLevel = null;
            if (PronomToPdfAConformanceLevel.ContainsKey(files.First().TargetPronom))
            {
                conformanceLevel = PronomToPdfAConformanceLevel[files.First().TargetPronom];
            }
            ...
            if (result != null)
            {
                ...
                newFileInfo.IsMerged = files.First().TargetPronom == result.matches[0].id;
